...
{
    // Replaces GarageContextInitializer (DropCreateDatabaseIfModelChanges).
    // From here on, a schema change means running Add-Migration + Update-
    // Database instead of wiping the whole database - real customer data
    // survives a model change instead of being deleted by it.
    internal sealed class Configuration : DbMigrationsConfiguration<GarageContext>
    {
        ...
        // Only seeds a genuinely empty database (first-ever deployment) -
        // running Update-Database again against a database that already has
        // real data in it just does nothing here, instead of re-adding demo
        // accounts or duplicating rows.
